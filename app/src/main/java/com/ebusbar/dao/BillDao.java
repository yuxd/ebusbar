 ...
 */
public class BillDao implements Parcelable {
    ...
    protected BillDao(Parcel in) {
        this.crm_balancelog_get = in.readParcelable(CrmBalancelogGetEntity.class.getClassLoader());
    }

    public static final Parcelable.Creator<BillDao> CREATOR = new Parcelable.Creator<BillDao>() {
        @Override
        public BillDao createFromParcel(Parcel source) {
            return new BillDao(source);
        }

        @Override
        public BillDao[] newArray(int size) {
            return new BillDao[size];
        }
    };
}
