 ...
 */
public class AllStation extends Search implements Parcelable {
    ...
    protected AllStation(Parcel in) {
        this.evc_stations_getall = in.readParcelable(EvcStationsGetallEntity.class.getClassLoader());
    }

    public static final Parcelable.Creator<AllStation> CREATOR = new Parcelable.Creator<AllStation>() {
        @Override
        public AllStation createFromParcel(Parcel source) {
            return new AllStation(source);
        }

        @Override
        public AllStation[] newArray(int size) {
            return new AllStation[size];
        }
    };
}
